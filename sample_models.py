...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = GRU(units, activation='relu',
                       return_sequences=True, implementation=2, name='rnn' + str(i))(r_layers[-1])
        bn_rnn = BatchNormalization()(simp_rnn)
        r_layers.append(bn_rnn)
        
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(r_layers[-1])
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(LSTM(units, implementation=2, return_sequences=True), 
